...
DOCS="$ROOT/docs"
OUT="$ROOT/AQ_Reference_PDF_test_v5.pdf"
COMBINED="$DOCS/_pdf_test_v5.html"
...
for i, (rel, short, label) in enumerate(pages):
    ...
    # docs/_pdf_test_v5.html rather than in docs/tables/*.html.
...
<style>
  ...
  @media print {
    ...
    table.docutils th:nth-child(2), table.docutils td:nth-child(2) { width: 18% !important; }
    table.docutils th:nth-child(3), table.docutils td:nth-child(3) { width: 13% !important; }
    table.docutils th:nth-child(4), table.docutils td:nth-child(4) { width: 13% !important; }
    table.docutils th:nth-child(5), table.docutils td:nth-child(5) { width: 9% !important; }
    table.docutils th:nth-child(6), table.docutils td:nth-child(6) { width: 13% !important; }
...
echo
echo "Creating v5 PDF test with Chrome..."
"$CHROME" \
  --headless \
  --disable-gpu \
...
echo "$OUT"
echo
echo "v5 keeps the v1/v3 rendering method and adds:"
echo "  - the Tables index page before the individual table pages
  - restored RTD card styling, including Reference-only dark cards
  - internal links between included pages"
echo "  - fixed-width attribute tables so the final columns stay inside the page"
