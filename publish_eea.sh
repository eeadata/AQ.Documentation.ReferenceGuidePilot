...
STATIC_PDF="$SCRIPT_DIR/source/_static/AQ_eReference_Guide.pdf"
DOCS_PDF="$DOCS_DIR/_static/AQ_eReference_Guide.pdf"

CHECK_DIR="$SCRIPT_DIR/build/eea-check"
CHECK_PDF="$CHECK_DIR/_static/AQ_eReference_Guide.pdf"
...
[[ -s "$STATIC_PDF" ]] || \
    fail "source/_static/AQ_eReference_Guide.pdf is missing or empty."

[[ -s "$DOCS_PDF" ]] || \
    fail "docs/_static/AQ_eReference_Guide.pdf is missing or empty."
...
git ls-files --error-unmatch "source/_static/AQ_eReference_Guide.pdf" >/dev/null 2>&1 || \
    fail "source/_static/AQ_eReference_Guide.pdf is not tracked by Git."

git ls-files --error-unmatch "docs/_static/AQ_eReference_Guide.pdf" >/dev/null 2>&1 || \
    fail "docs/_static/AQ_eReference_Guide.pdf is not tracked by Git."

echo "PDF source copy : source/_static/AQ_eReference_Guide.pdf"
echo "PDF website copy: docs/_static/AQ_eReference_Guide.pdf"
...
[[ -s "$CHECK_PDF" ]] || \
    fail "The strict validation build does not contain _static/AQ_eReference_Guide.pdf."

cmp -s "$STATIC_PDF" "$CHECK_PDF" || \
    fail "The PDF copied by the strict validation build differs from source/_static/AQ_eReference_Guide.pdf."
...
echo "  - the HTML AQ eReference Guide"
echo "  - the PDF version at _static/AQ_eReference_Guide.pdf"
echo
